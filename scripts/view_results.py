...
2. Scatter: wind_24h    — predicted vs actual (coloured by basin)
3. Scatter: wind_48h    — predicted vs actual (coloured by basin)
4. Decay curves         — mean ± std predicted decay path per basin
5. Error distributions  — residual histograms for 24 h and 48 h
6. Spatial error map    — mean |error| vs landfall lat/lon (if available)

Usage
─────
  # View results from the best checkpoint
  python scripts/view_results.py

  # Specify a different checkpoint
  python scripts/view_results.py --ckpt models/last_ufno.pt

  # Also show interactive matplotlib windows (blocks until closed)
...
def main():
    ...
    parser.add_argument("--ckpt", default=os.path.join(
                            os.path.dirname(__file__), "..", "models", "best_ufno.pt"))
    ...
    if not os.path.exists(args.ckpt):
        print(f"No checkpoint found at {args.ckpt}.\n"
              "Run train_ufno.py first.")
        sys.exit(1)

    print(f"Loading checkpoint : {args.ckpt}")
    model, meta, ckpt = load_model(args.ckpt)

    # Load history if available
    hist_path = os.path.join(os.path.dirname(args.ckpt), "ufno_history.json")
